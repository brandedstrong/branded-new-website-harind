...
const TextMarquee: React.FC<TextMarqueeProps> = ({ className = "" }) => {
    ...
    const marqueeText = "FEWER TOOLS TO JUGGLE PRO TEAM ON DEMAND NO DESIGN DEBT SAVE 10+ HRS/WEEK";
    const upperBandText = "FEWER TOOLS TO JUGGLE PRO TEAM";

    return (
        <div ref={marqueeRef} className={`relative sm:-top-[300px] -top-[270px] z-10 overflow-hidden  ${className}`} role="banner" aria-label="Promotional text marquee">
            {/* Background with large faded text */}


            {/* Diagonal Bands Container */}
            <div className="relative z-10 h-[100px] xs:h-[120px] sm:h-[140px] md:h-[160px] lg:h-[180px] xl:h-[200px] 2xl:h-[220px] overflow-hidden marquee-container">

                {/* Lower Lime Green Band */}
                <div className="absolute bottom-0 left-0 py-3 w-full bg-gradient-to-r from-[#a3e635] to-[#facc15] transform  origin-left"
                ...
                >
                    <div className="flex items-center h-full overflow-hidden">
                        <div className="flex animate-marquee-fast whitespace-nowrap text-black font-bold text-[10px] xs:text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl">
                            <span className="mr-3 xs:mr-4 sm:mr-6 md:mr-8 lg:mr-10 xl:mr-12 2xl:mr-14">{marqueeText}</span>
                            <span className="mr-3 xs:mr-4 sm:mr-6 md:mr-8 lg:mr-10 xl:mr-12 2xl:mr-14">{marqueeText}</span>
                            <span className="mr-3 xs:mr-4 sm:mr-6 md:mr-8 lg:mr-10 xl:mr-12 2xl:mr-14">{marqueeText}</span>
                            <span className="mr-3 xs:mr-4 sm:mr-6 md:mr-8 lg:mr-10 xl:mr-12 2xl:mr-14">{marqueeText}</span>
                            <span className="mr-3 xs:mr-4 sm:mr-6 md:mr-8 lg:mr-10 xl:mr-12 2xl:mr-14">{marqueeText}</span>
