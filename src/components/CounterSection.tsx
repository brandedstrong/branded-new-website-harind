...
const CounterSection = () => {
  ...
  // Get animated values for each statistic
  const animatedValues = statistics.map((stat) =>
    useCounterAnimation({
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url(${IMAGES.Power_Img})` }}
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div
          ref={elementRef}
          className="max-w-6xl mx-auto bg-center bg-cover p-6 md:p-16 rounded-[0px] sm:rounded-[30px]"
          style={{ backgroundImage: `url(${IMAGES.Glans_Img})` }}
        >
          {/* Main Heading */}
          <div className="text-center mb-12 lg:mb-16">
            <h1 className="hidden sm:block sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight uppercase">
              ...
              <br className="hidden md:block" /> VIDEO MARKETING
            </h1>
            <h1 className="block sm:hidden text-5xl font-bold text-white leading-tight uppercase">
              ...
              <br />
              <span className="font-light">OF</span>{" "}
              <br className="hidden md:block" /> VIDEO <br /> MARKETING
            </h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8 lg:mb-10">
            {statistics.map((stat, index) => (
              <div key={index} className="text-center">
                ...
                >
                  {index === 3
                    ? `${animatedValues[index]}X`
                    : `${animatedValues[index]}%`}
                </div>

                {/* Primary Descriptor */}
                <div
                  className={`text-sm text-center sm:text-base md:text-lg lg:text-xl font-semibold text-white uppercase mb-1 max-w-[150px] mx-auto ${
                    index === 1
                      ? "sm:!max-w-[100px] !max-w-[80px]"
                      : "!max-w-[150px]"
                  }`}
                >
                  {stat.primaryText}
